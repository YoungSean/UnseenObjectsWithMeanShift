...
def hypersphere_attention(
    ...
    # attn = kappa * (1 + kappa_weight) * attn
    attn = kappa * attn
    #attn = torch.exp(kappa * attn)
    if attn_mask is not None:
        attn += attn_mask
    attn = F.softmax(attn, dim=-1)
    if dropout_p > 0.0:
        attn = F.dropout(attn, p=dropout_p)
    # update the cluster centers / new means
...
class MeanShiftAttention(nn.MultiheadAttention):
    ...
    def __init__(self, embed_dim, num_heads=1, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        # # Generally we just set num_heads as one.
...
# Mean shift cross attention part
# def _exp_dot_product_attention(
#     q: Tensor,
#     k: Tensor,
#     v: Tensor,
#     attn_mask: Optional[Tensor] = None,
...
#     if attn_mask is not None:
#         attn += attn_mask
#         print("Attention mask is invalid when using the mean shift attention.")
#
#     if dropout_p > 0.0:
#         attn = F.dropout(attn, p=dropout_p)
#     # update the cluster centers / new means
...
# def mean_shift_attention_forward(
#     query: Tensor,
#     key: Tensor,
#     value: Tensor,
#     embed_dim_to_check: int,
...
#     # prep key padding mask
#     if key_padding_mask is not None and key_padding_mask.dtype == torch.uint8:
#         print("Key padding mask is invalid when using the mean shift attention.")
#
#     # add bias along batch dimension (currently second)
#     if bias_k is not None and bias_v is not None:
#         assert static_k is None, "bias cannot be added to static key."
...
#     # (deep breath) calculate attention and out projection
#     #
#     #print("before exp attention", q[0])
#     attn_output, attn_output_weights = _exp_dot_product_attention(q, k, v, attn_mask, dropout_p)
#     #print("exp attention output", attn_output)
#     attn_output = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, embed_dim)
#     attn_output = F.linear(attn_output, out_proj_weight, out_proj_bias)
#
#     if need_weights:
